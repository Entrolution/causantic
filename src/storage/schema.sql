...
-- Persisted HDBSCAN models for incremental clustering
...
CREATE TABLE IF NOT EXISTS entities (
  id TEXT PRIMARY KEY,
  entity_type TEXT NOT NULL,      -- 'person' | 'channel' | 'meeting' | 'organization' | 'url'
  canonical_name TEXT NOT NULL,
  project_slug TEXT NOT NULL,
  created_at TEXT DEFAULT CURRENT_TIMESTAMP,
  updated_at TEXT DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_entities_project ON entities(project_slug);
CREATE INDEX IF NOT EXISTS idx_entities_type_project ON entities(entity_type, project_slug);

-- Entity alias lookup (multiple surface forms → one entity)
CREATE TABLE IF NOT EXISTS entity_aliases (
  alias TEXT NOT NULL,            -- Normalized: "joel", "#general"
  entity_id TEXT NOT NULL,
  entity_type TEXT NOT NULL,      -- Disambiguates: "alice" as person vs channel
  project_slug TEXT NOT NULL,
...
-- Entity mentions in chunks (many-to-many)
...
-- Insert initial version if not exists (v16 adds entity tables)
