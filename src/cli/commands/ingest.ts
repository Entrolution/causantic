...
export const ingestCommand: Command = {
  ...
  usage: 'causantic ingest <path> [--force]',
  handler: async (args) => {
    if (args.length === 0) {
      console.error('Error: Path required');
      console.log('Usage: causantic ingest <path>');
      process.exit(2);
    }
...
export const batchIngestCommand: Command = {
  ...
  usage: 'causantic batch-ingest <directory> [--parallel <n>]',
  handler: async (args) => {
    if (args.length === 0) {
      console.error('Error: Directory required');
      console.log('Usage: causantic batch-ingest <directory>');
      process.exit(2);
    }
