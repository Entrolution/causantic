...
**Date**: 2026-02-03
**Status**: Benchmark Complete (two runs)
...
Only `minClusterSize=3` was tested. Different values would produce different noise ratios and cluster counts. The high noise ratios may partially reflect this configuration choice rather than model limitations.
...
The drift comparison between bge-small and nomic-v1.5 cannot isolate truncation effects from architecture differences. A same-model truncation test (e.g., jina-small with vs without truncation) would be more informative.
...
2. **Same-model truncation test**: Run jina-small with text truncated to 512 tokens vs full text to directly measure context window value
3. **Tune HDBSCAN**: Sweep `minClusterSize` (2-10) and `minSamples` to find optimal clustering density for jina-small
4. **Human validation**: Manually inspect 20-30 pairs to calibrate auto-labeler accuracy
5. **Boilerplate filtering**: Pre-strip "This session is being continued..." and other boilerplate before embedding — measure impact on all models
6. **Integration prototype**: Wire jina-small into a SessionEnd hook prototype and measure end-to-end latency
7. **Matryoshka test**: Try nomic-v1.5 at reduced dimensions (384/256) to see if it improves — low priority given overall poor performance
